    ...
    for el in data['elements']:
        ...
        if center:
            ...
            if lat_center and lon_center:
                points.append(f"Walkable Area at ({lat_center}, {lon_center}) [highway={el['tags'].get('highway', 'unknown')}]")
    return points
...
def Main():
    ...
    if len(pois) < 15:
        print(f"Found only {len(pois)} POIs, switching to walkable area mode.")
        pois = FindWalkableAreas(coords[0], coords[1], radius)

    if not pois:
        print("No POIs or walkable areas found.")
        return
