...
def Main():
    ...
    with open("privacy_utility_metrics.csv", "w", encoding="utf-8") as metrics_file:
        ...
        for run in range(1, num_runs + 1):
            ...
            # Calculate utility for this location
            utility = calculate_distance(
                coords[0], coords[1], float(chosen_location[1]), float(chosen_location[2])
            )
            
            # Calculate privacy (distance to centroid of all locations selected so far)
            ...
            print(f"  - Privacy (with {run} locations): {privacy:.4f} km")
    ...
    final_avg_distance = calculate_average_distance(coords[0], coords[1], chosen_locations)
    final_privacy_distance = calculate_privacy_distance(coords[0], coords[1], chosen_locations)
    
    # Save aggregated results to file
    with open("suggested_locations.txt", "w", encoding="utf-8") as file:
        file.write("FINAL AGGREGATED METRICS:\n")
        file.write(f"Average Utility: {final_avg_distance:.4f} km\n")
        file.write(f"Final Privacy: {final_privacy_distance:.4f} km\n\n")
