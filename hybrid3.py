...
def load_previous_locations():
    """Load previously selected locations from a history file."""
    previous_locations = []
    try:
        with open("location_history.pkl", "rb") as f:
            import pickle
            previous_locations = pickle.load(f)
        print(f"Loaded {len(previous_locations)} previously selected locations.")
    except (FileNotFoundError, EOFError, pickle.UnpicklingError) as e:
        print(f"No previous location history found or error reading history: {e}")
        print("Starting with empty location history.")
    
    return previous_locations

def save_location_history(locations):
    """Save the current set of selected locations to history file."""
    import pickle
    with open("location_history.pkl", "wb") as f:
        pickle.dump(locations, f)
    print(f"Saved {len(locations)} locations to history file.")
...
def Main():
    ...
    previously_selected_locations = load_previous_locations()
    print(f"Starting with {len(previously_selected_locations)} previously selected locations.")
    ...
    for loc in locations_to_use:
        key = f"{loc[0]} at ({loc[1]}, {loc[2]}) [{loc[3]}]"
    ...
    with open("privacy_utility_metrics.csv", "w", encoding="utf-8") as metrics_file:
        metrics_file.write("Run,Location,Utility(km),Privacy(km),CumulativePrivacy(km)\n")
        
        # Maintain a list of newly chosen locations for this execution
        newly_chosen_locations = []
        location_counter = defaultdict(int)
        
        # Working set of all locations (previous + new)
        all_chosen_locations = previously_selected_locations.copy()
        ...
        for run in range(1, num_runs + 1):
            ...
            # Add to list of newly chosen locations for this run
            newly_chosen_locations.append(chosen_location)
            
            # Add to the working set of all locations
            all_chosen_locations.append(chosen_location)
            
            # Calculate metrics for this individual run
            utility = calculate_distance(
                coords[0], coords[1], float(chosen_location[1]), float(chosen_location[2])
            )
            
            # Privacy compared to just the previous locations (before this run)
            previous_privacy = 0
            if len(previously_selected_locations) > 0:
                previous_privacy = calculate_privacy_distance(
                    coords[0], coords[1], previously_selected_locations
                )
            
            # Calculate cumulative privacy (distance to centroid of ALL chosen locations)
            cumulative_privacy = calculate_privacy_distance(
                coords[0], coords[1], all_chosen_locations
            )
            
            # Write metrics to file for this run
            metrics_file.write(
                f"{run},{chosen_key},{utility:.4f},{previous_privacy:.4f},{cumulative_privacy:.4f}\n"
            )
            
            print(f"Run {run}: Selected {chosen_location[0]}")
            print(f"  - Utility: {utility:.4f} km")
            print(f"  - Privacy compared to previous locations: {previous_privacy:.4f} km")
            print(f"  - Cumulative Privacy with all locations: {cumulative_privacy:.4f} km")
    
    # Create the map with the user location and ALL locations (previous + new)
    CreateMap(coords[0], coords[1], radius, locations_to_use, location_counter)
    
    # Calculate final metrics for all chosen locations
    final_avg_distance = calculate_average_distance(coords[0], coords[1], newly_chosen_locations)
    final_privacy_distance = calculate_privacy_distance(coords[0], coords[1], all_chosen_locations)
    previous_privacy_distance = calculate_privacy_distance(coords[0], coords[1], previously_selected_locations) if previously_selected_locations else 0
    
    # Save aggregated results to file
    with open("suggested_locations.txt", "w", encoding="utf-8") as file:
        file.write("FINAL AGGREGATED METRICS:\n")
        file.write(f"Average Utility (this run): {final_avg_distance:.4f} km\n")
        file.write(f"Privacy with previous locations: {previous_privacy_distance:.4f} km\n")
        file.write(f"Final Privacy with all locations: {final_privacy_distance:.4f} km\n\n")
        
        file.write(f"Previously selected locations: {len(previously_selected_locations)}\n")
        file.write(f"Newly selected locations: {len(newly_chosen_locations)}\n")
        file.write(f"Total locations in history: {len(all_chosen_locations)}\n\n")
        
        file.write("Locations selected in this run:\n")
        for loc_key, count in location_counter.items():
            file.write(f"{loc_key} -> suggested {count} times\n")
    
    # Save the updated location history
    save_location_history(all_chosen_locations)
    
    print("\nDone! Map and results files have been created.")
    print("Output files:")
    print("Map.html: Interactive map with user location and suggested locations")
    print("suggested_locations.txt: List of suggested locations with aggregated metrics")
    print("privacy_utility_metrics.csv: Detailed per-run privacy and utility metrics")
    print("location_history.pkl: Updated history of all selected locations")
